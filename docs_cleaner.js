...
class DocsCleaner {
    ...
    analyzeProjectDocs() {
        ...
        const docs = this.findProjectDocs();
    ...
    analyzeDocValue(docs) {
        ...
        docs.forEach(doc => {
            ...
            if (fileName.includes('summary') || fileName.includes('Summary')) score += 2;
...
// 主函数
async function main() {
    const cleaner = new DocsCleaner();
    
    const args = process.argv.slice(2);
    const command = args[0];
    
    if (command === 'optimize') {
        const result = await cleaner.optimizeDocs(false);
        cleaner.generateNewDocStructure();
        ...
        const result = await cleaner.optimizeDocs(true);
        cleaner.generateNewDocStructure();
        
        console.log('\n💡 运行 "node docs_cleaner.js optimize" 来执行实际优化');
    }
}
